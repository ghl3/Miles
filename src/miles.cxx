...
std::unique_ptr<crow::SimpleApp>
createServer(const std::shared_ptr<Database> &storage) {

    auto app = std::make_unique<crow::SimpleApp>();

    app->route_dynamic("/")([]() { return "Hello world"; });

    app->route_dynamic("/store/<str>/<str>")
        .methods("POST"_method)([=](const crow::request &req,
            ...
            try {
                ...
                auto storageResult =
                    storage->store(table, key, std::move(payload));
                LOG(INFO) << "Storage result: " << storageResult.isSuccess
                          << std::endl;

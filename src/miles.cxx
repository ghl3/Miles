...
std::unique_ptr<crow::SimpleApp> createServer(const std::shared_ptr<Database>& storage) {
    ...
    app->route_dynamic("/store/<str>/<str>")
        .methods("POST"_method)([=](const crow::request& req, const std::string table, const std::string key) {
            try {
    ...
    app->route_dynamic("/add_json").methods("POST"_method)([](const crow::request& req) {
